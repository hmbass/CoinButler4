...
def get_database_connection():
   ...
   try:
       ...
       if cursor.fetchone()[0] == 0:
           ...
           conn.commit()
           st.success("✅ 데이터베이스가 자동으로 초기화되었습니다!")
       
       return conn
       
   except Exception as e:
       st.error(f"❌ 데이터베이스 연결 오류: {e}")
       # 오류 발생 시 더미 데이터 반환을 위한 빈 연결 반환
       return None
...
def load_recent_trades():
   try:
       ...
       st.warning(f"데이터베이스 연결 오류: {e}")
       # 더미 데이터 반환
       return pd.DataFrame({
           'timestamp': [datetime.now()],
           'decision': ['HOLD'],
           'percentage': [65.5],
           'reason': ['시뮬레이션 모드 - 데이터베이스 연결 대기 중'],
           'btc_balance': [0.001],
           'krw_balance': [50000.0],
           'btc_avg_buy_price': [45000000.0],
           'btc_krw_price': [45000000.0]
       })
...
def load_reflections():
   try:
       ...
       st.warning(f"반성 일기 데이터베이스 연결 오류: {e}")
       # 더미 반성 데이터 반환
       return pd.DataFrame({
           'reflection_date': [datetime.now()],
           'reflection_text': ['시뮬레이션 모드에서 실행 중입니다.'],
           'mood_score': [7],
           'learning_points': ['데이터베이스 연결 확인 필요'],
           'next_actions': ['API 키 설정 후 재시작'],
           'decision': ['HOLD'],
           'percentage': [65.5],
           'btc_krw_price': [45000000.0]
       })
